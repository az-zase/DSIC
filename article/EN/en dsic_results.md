...
**Script:** `dsic_test.py` (single, self-contained; blocks `[0]`–`[11]`)
...
> **On run time.** A full run takes roughly 10–15 minutes: downloading the supernova catalog (~65 MB), the Pantheon+SH0ES fits (a few minutes), and block `[11]` (structure growth) — the longest stage (a grid scan with numerical ODE integration at every node, ~5 minutes). Between the printout of block `[10]` and the results of `[11]` there may be a long silent pause — this is expected; the script is computing, no need to interrupt it.
