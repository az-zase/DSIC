...
> **The matching point is individual (§11.2 vs §11.4).** The form of the clock is one and the same in both places: `C·arccos μ` in the normal phase and `K·ln(1/μ)` in the relative one. What differs is *where the rate of the clock is matched*. The background of §11.2 switches at the common threshold, so its amplitude is `C = K·√(1−μ_d²)/μ_d`. A separate island switches at its own detachment, so its amplitude is `C_f = K·√(1−μ_f²)/μ_f` with `μ_f = μ(z_form)`. The difference is not cosmetic: it is forced by the locality of detachment (§9, §11.3). Were an island matched at `μ_d`, it would switch its clock not when it detached itself but when the background did — which contradicts the island picture. In the limit `z_form → z_detach` the island measure converges continuously to the background curve of §11.2.

The redshifts of the objects are spectroscopic and the masses are the virial/accretion estimates of the discovery publications [16–23] (block `[6b]`):
